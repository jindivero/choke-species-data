...
m_8 <- sdmTMB(formula = O2_umolkg  ~ 0  +as.factor(survey) + s(sigma0_exp)+ s(temperature_C) +  s(depth_ln) + s(doy),
              mesh = spde,
              data = train_data, 
              family = gaussian(), 
              time = "year",
...
save_model = TRUE
if (save_model) {
  save(x = output, file = "code/compare_predicted_actual.Rdata")
...
if (!fitmodel) {
  load(file = "code/compare_predicted_actual.Rdata")
  plot_string <-"figures/test_predictions/wcbts_predict_partial.pdf"
  pdf(plot_string, onefile = TRUE)
