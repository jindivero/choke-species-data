...
temp_test <- tibble(year = 2010,
                    sigma0_kgm3 = mean(train_data$sigma0_kgm3),
                    temperature_C = seq(min(train_data$temperature_C), max(train_data$temperature_C), length.out = 100),
                    depth_ln = mean(train_data$depth_ln),
                    doy = mean(train_data$doy),
                    Y = mean(train_data$Y),
                    X = mean(train_data$X))
depth_test <- tibble(year = 2010,
                     sigma0_kgm3 = mean(train_data$sigma0_kgm3),
                     temperature_C = mean(train_data$temperature_C),
                     depth_ln = seq(min(train_data$depth_ln), max(train_data$depth_ln), length.out = 100),
                     doy = mean(train_data$doy),
                     Y = mean(train_data$Y),
                     X = mean(train_data$X))
doy_test <- tibble(year = 2010,
                   ...
                   temperature_C = mean(train_data$temperature_C),
                   depth_ln = mean(train_data$depth_ln),
                   doy = seq(min(train_data$doy), max(train_data$doy), length.out = 100),
                   Y = mean(train_data$Y),
                   X = mean(train_data$X))
sigma0_test <- tibble(year = 2010,
                      sigma0_kgm3 = seq(min(train_data$sigma0_kgm3), max(train_data$sigma0_kgm3), length.out = 100),
                      temperature_C = mean(train_data$temperature_C),
                      depth_ln = mean(train_data$depth_ln),
                      doy = mean(train_data$doy),
                      Y = mean(train_data$Y),
...
ggplot(us_coast_proj) + geom_sf() +
  geom_point(data = train_data, aes(x = X * 1000, y = Y * 1000, col = sigma0_kgm3),  size = 1.0, alpha = 0.5) +
