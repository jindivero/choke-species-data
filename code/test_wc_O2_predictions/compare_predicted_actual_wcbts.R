...
# Loop through years ####
if (fitmodel) {
  plot_string <-"figures/test_predictions/wcbts_predict.pdf"
  ...
  for (i in 1:length(yearlist)) {
    ...
    m_8 <- sdmTMB(formula = O2_umolkg  ~ 0  + as.factor(survey) + s(sigma0_exp)+ s(temperature_C, k = 4) +  s(depth_ln) + s(doy),
                  mesh = spde,
                  data = train_data, 
                  family = gaussian(), 
                  time = "year",
  ...
  save_model = TRUE
  if (save_model) {
    save(x = output, file = "code/test_wc_O2_predictions/compare_predicted_actual_full_year.Rdata")
...
if (!fitmodel) {
  load(file = "code/test_wc_O2_predictions/compare_predicted_actual_full_year.Rdata")
  plot_string <-"figures/test_predictions/wcbts_predict.pdf"
  pdf(plot_string, onefile = TRUE)
