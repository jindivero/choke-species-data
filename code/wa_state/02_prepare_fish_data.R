...
#Make list
sci_names <- c("gadus chalcogrammus", "anoplopoma fimbria", "microstomus pacificus", "eopsetta jordani", "gadus macrocephalus", "sebastolobus altivelis", "sebastolobus alascanus", "hippoglossus stenolepis", "sebastes pinniger", "ophiodon elongatus", "sebastes crameri", "atheresthes stomias", "merluccius productus", "sebastes entomelas", "squalus suckleyi","sebastes ruberrimus","sebastes maliger", "glyptocephalus zachirus", "hippoglossoides elassodon", "parophrys vetulus", "lepidopsetta bilineata", "sebastes aleutianus")
spcs <- c("walleye pollock", "sablefish", "dover sole", "petrale sole", "pacific cod", "longspine thornyhead", "shortspine thornyhead", "pacific halibut", "canary rockfish", "lingcod", "darkblotched rockfish",  "arrowtooth flounder", "pacific hake", "widow rockfish", "pacific spiny dogfish", "yelloweye rockfish", "quillback rockfish", "rex sole", "flathead sole", "english sole", "southern rock sole", "rougheye rockfish")

#Apply to multiple species
dataframes3 <- list()
for(i in 12:length(sci_names)){
  message( "Preparing ", spcs[i], Sys.time() )
  dat <- prepare_data(spcs[i], sci_names[i])
  name <- paste(spcs[i])
  dataframes3[[name]] <- dat
}
